...
For any input, jsesc generates the shortest possible valid printable-ASCII-only output. [Here鈥檚 an online demo.](https://mothereff.in/js-escapes)

jsesc鈥檚 output can be used instead of `JSON.stringify`鈥檚 to avoid [mojibake](https://en.wikipedia.org/wiki/Mojibake) and other encoding issues, or even to [avoid errors](https://twitter.com/annevk/status/380000829643571200) when passing JSON-formatted data (which may contain U+2028 LINE SEPARATOR, U+2029 PARAGRAPH SEPARATOR, or [lone surrogates](https://esdiscuss.org/topic/code-points-vs-unicode-scalar-values#content-14)) to a JavaScript parser or an UTF-8 encoder.
...
// Escaping an array
jsesc([
  'Ich 鈾?B眉cher', 'foo 饾寙 bar'
]);
// 鈫?'[\'Ich \\u2665 B\\xFCcher\',\'foo \\uD834\\uDF06 bar\']'
...
// 鈫?'Lorem ipsum "dolor" sit \\\'amet\\\' etc.'
...
The `es6` option takes a boolean value (`true` or `false`), and defaults to `false` (disabled). When enabled, any astral Unicode symbols in the input are escaped using [ECMAScript 6 Unicode code point escape sequences](https://mathiasbynens.be/notes/javascript-escapes#unicode-code-point) instead of using separate escape sequences for each surrogate half. If backwards compatibility with ES5 environments is a concern, don鈥檛 enable this setting. If the `json` setting is enabled, the value for the `es6` setting is ignored (as if it was `false`).

```js
// By default, the `es6` option is disabled:
jsesc('foo 饾寙 bar 馃挬 baz');
// 鈫?'foo \\uD834\\uDF06 bar \\uD83D\\uDCA9 baz'
...
// 鈫?'foo \\u{1D306} bar \\u{1F4A9} baz'
```

#### `escapeEverything`

The `escapeEverything` option takes a boolean value (`true` or `false`), and defaults to `false` (disabled). When enabled, all the symbols in the output are escaped 鈥?even printable ASCII symbols.

```js
jsesc('lolwat"foo\'bar', {
  'escapeEverything': true
});
// 鈫?'\\x6C\\x6F\\x6C\\x77\\x61\\x74\\"\\x66\\x6F\\x6F\\\'\\x62\\x61\\x72'
// 鈫?"\\x6C\\x6F\\x6C\\x77\\x61\\x74\\\"\\x66\\x6F\\x6F\\'\\x62\\x61\\x72"
```
...
The `isScriptContext` option takes a boolean value (`true` or `false`), and defaults to `false` (disabled). When enabled, occurrences of [`</script` and `</style`](https://mathiasbynens.be/notes/etago) in the output are escaped as `<\/script` and `<\/style`, and [`<!--`](https://mathiasbynens.be/notes/etago#comment-8) is escaped as `\x3C!--` (or `\u003C!--` when the `json` option is enabled). This setting is useful when jsesc鈥檚 output ends up as part of a `<script>` or `<style>` element in an HTML document.
...
The `compact` option takes a boolean value (`true` or `false`), and defaults to `true` (enabled). When enabled, the output for arrays and objects is as compact as possible; it鈥檚 not formatted nicely.
...
// Values that are acceptable in JSON but aren鈥檛 strings, arrays, or object
// literals can鈥檛 be escaped, so they鈥檒l just be preserved:
jsesc([ 'foo\x00bar', [1, '漏', { 'foo': true, 'qux': null }], 42 ], {
  'json': true
});
// 鈫?'["foo\\u0000bar",[1,"\\u00A9",{"foo":true,"qux":null}],42]'
// Values that aren鈥檛 allowed in JSON are run through `JSON.stringify()`:
...
**Note:** Using this option on objects or arrays that contain non-string values relies on `JSON.stringify()`. For legacy environments like IE 鈮?7, use [a `JSON` polyfill](http://bestiejs.github.io/json3/).
...
After that you鈥檙e able to escape strings from the command line:

```bash
$ jsesc 'f枚o 鈾?b氓r 饾寙 baz'
f\xF6o \u2665 b\xE5r \uD834\uDF06 baz
```
...
```bash
$ jsesc --json --pretty '{ "f枚o": "鈾?, "b氓r": "饾寙 baz" }'
{
  "f\u00F6o": "\u2665",
  "b\u00E5r": "\uD834\uDF06 baz"
}
